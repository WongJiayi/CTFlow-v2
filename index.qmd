...
#| fig-cap: "FVD across the submission cycle's real, scored runs (lower is better). Marker size ~ FID avg where available."
#| fig-width: 9
#| fig-height: 5
...
fig, ax = plt.subplots(figsize=(9, 5))
sizes = df["fid_avg"].fillna(60).astype(float) * 3
colors = ["#e74c3c" if "bug" in n.lower() or "broken" in n.lower() else "#2ecc71" if "best" in n.lower() else "#7f8c8d"
          for n in df["label"]]
ax.plot(x, df["fvd"], color="#7f8c8d", linewidth=1, zorder=1, linestyle="--")
ax.scatter(x, df["fvd"], s=sizes, c=colors, zorder=2, edgecolor="white", linewidth=0.5)

for i, row in df.iterrows():
    ax.annotate(row["label"], (i, row["fvd"]), textcoords="offset points",
                xytext=(0, 12), ha="center", fontsize=8, wrap=True)

ax.set_ylabel("FVD (CTNet features, lower is better)", fontsize=12)
ax.set_xticks(list(x))
ax.set_xticklabels([f"submission {i+1}" for i in x], fontsize=9)
ax.spines[["top", "right"]].set_visible(False)
ax.set_ylim(0, df["fvd"].max() * 1.25)
plt.tight_layout()
plt.show()
```

Green = the cycle's best result; red = a run that scored badly for a
real, identified reason; grey = intermediate points. The dip and
recovery in the middle is the Dockerfile-ENV-mismatch and
degenerate-candidate bugs described below; the spike on the right is the
CLIP-safety-net bug, also described below — both are real regressions
with real root causes, not noise.
