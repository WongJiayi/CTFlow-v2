...
that scored badly for a real, identified reason; grey = intermediate
reference points. FID and CLIP weren't logged for the very first
(pre-cycle) and the Dockerfile-ENV-mismatch runs, so those two panels are
missing points 1 and 3.

Three things worth reading off this directly rather than from FVD alone.
First, submission 6 (CLIP safety net silently broken) vs. submission 5
(session best): FVD alone makes 6 look like a step backward, but FID
...
strikingly: submission 4 (`xy=1.0mm`) has the single best FVD of the
entire cycle (0.243) *and* by far the worst FID (565, roughly 6x every
...
submission 7, the one that actually shipped once the offline-tokenizer
...
which submission 6 never demonstrated at all.
