...
x = range(len(df))
colors = ["#e74c3c" if "bug" in n.lower() or "broken" in n.lower() else "#2ecc71" if "best" in n.lower() else "#7f8c8d"
          for n in df["label"]]
...
fig.legend(handles=[
    plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="#2ecc71", markersize=10, label="best result"),
...
Green = a best-FVD-so-far result; red = a run that scored badly for a
real, identified reason; grey = intermediate points. FID and CLIP weren't
logged for the very first (pre-cycle) and the Dockerfile-ENV-mismatch
runs, so those two panels are missing points 1 and 3.

Two things worth reading off this directly rather than from FVD alone.
First, submission 6 (final) vs. submission 5 (session best): FVD alone
makes it look like a step backward, but FID barely moved (90.2 → 92.9)
and CLIP actually *improved slightly* (28.5 → 29.7) — the regression is
concentrated in FVD specifically, consistent with the root cause (a
diagnostic, feature-space-distribution issue caused by the broken CLIP
safety net) rather than a wholesale quality collapse. Second, and more
...
just one being noisier than the other. That one's explained below.
