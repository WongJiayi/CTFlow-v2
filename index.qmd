...
metrics = [
    ("fvd",      "FVD  (↓ better)"),
    ("fid_avg",  "FID avg  (↓ better)"),
    ("clip_avg", "CT-CLIPScore avg  (↑ better)"),
]
...
for ax, (col, ylabel) in zip(axes, metrics):
    ...
    ax.scatter([i for i in x if have[i]], vals[have], s=140,
...
Green = the cycle's best FVD; red = a run that scored badly for a real,
identified reason; grey = intermediate points. FID and CLIP weren't
logged for the first three runs (before we started tracking per-metric
scores per submission), so those two panels only show the last two.
That's also the point worth making explicitly: FVD alone makes the last
submission look like a step backward from the session best, but FID
...
issue caused by the broken CLIP safety net) rather than a wholesale
quality collapse. The dip and recovery in the middle of the FVD panel is
the Dockerfile-ENV-mismatch and degenerate-candidate bugs described
below; the spike on the right is the CLIP-safety-net bug, also described
below — both are real regressions with real root causes, not noise.
